...
import { Bot, FileText, Terminal, Search, ChevronDown, ChevronUp, Loader2, CheckCircle2 } from 'lucide-react';
import { cn } from '../lib/utils';

interface AgentActivityPanelProps {
  activity: {
    type: 'thought' | 'file' | 'terminal' | 'web';
    message: string;
  }[];
  isLoading?: boolean;
}
...
export function AgentActivityPanel({ activity, isLoading }: AgentActivityPanelProps) {
  ...
  if (activity.length === 0 && !isLoading) return null;
      ...
      >
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          <div className="flex flex-col truncate">
            ...
            <span className="text-[11px] text-gray-500 dark:text-[#888] truncate">
              {lastActivity ? lastActivity.message : "Hazırlanıyor..."}
            </span>
          </div>
        </div>
                  ...
                  <div key={index} className="relative">
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <span className={cn(
                        ...
                        isLast && isLoading ? "text-gray-900 dark:text-[#e0e0e0] font-medium" : "text-gray-500 dark:text-[#888]"
                      )}>
                        {item.message}
                      </span>
                    </div>
